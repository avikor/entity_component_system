...
import pygame
from entity_component_system.component import GraphicsComponent, VelocityComponent, HorizontalOrientationComponent, \
...
def add_types_and_instantiate_entities(images: List[pygame.Surface], entities_manager: EntitiesManager,
...
def instantiate_alien(images: List[pygame.Surface], entities_manager: EntitiesManager, name_to_id_map: Dict[str, int]) \
        -> None:
    alien_type_id = entities_manager.add_entity_type([GraphicsComponent, AnimationCycleComponent, VelocityComponent])
    name_to_id_map["alien_type_id"] = alien_type_id
    alien_graphics_compo = GraphicsComponent(images[ImgsIndices.alien1], ALIEN_INITIAL_POSITION[0],
                                             ALIEN_INITIAL_POSITION[1])
    alien_ani_cycle_compo = AnimationCycleComponent((images[ImgsIndices.alien1], images[ImgsIndices.alien2],
                                                    images[ImgsIndices.alien3]), ALIEN_ANIMATION_INTERVAL_LENGTH)
    alien_velocity_compo = VelocityComponent(ALIEN_VELOCITY[0], ALIEN_VELOCITY[1])
...
def add_bomb_type(entities_manager: EntitiesManager, name_to_id_map: Dict[str, int]) -> None:
    name_to_id_map["bomb_type_id"] = entities_manager.add_entity_type([GraphicsComponent, VelocityComponent])


def instantiate_afv(images: List[pygame.Surface], entities_manager: EntitiesManager, name_to_id_map: Dict[str, int]) \
        -> None:
    name_to_id_map["afv_type_id"] = entities_manager.add_entity_type([GraphicsComponent, HorizontalOrientationComponent,
                                                                      VelocityComponent])
    afv_graphics_compo = GraphicsComponent(images[ImgsIndices.afv], AFV_INITIAL_POSITION[0], AFV_INITIAL_POSITION[1])
    afv_hori_ori_compo = HorizontalOrientationComponent(images[ImgsIndices.afv],
                                                        pygame.transform.flip(images[ImgsIndices.afv], True, False))
    afv_velocity_compo = VelocityComponent(AFV_VELOCITY[0], AFV_VELOCITY[1])
...
def add_shot_type(entities_manager: EntitiesManager, name_to_id_map: Dict[str, int]) -> None:
    name_to_id_map["shot_type_id"] = entities_manager.add_entity_type([GraphicsComponent, VelocityComponent,
                                                                       AudioComponent])


def add_explosion_type(entities_manager: EntitiesManager, name_to_id_map: Dict[str, int]) -> None:
    name_to_id_map["explosion_type_id"] = entities_manager.add_entity_type([GraphicsComponent, LifeTimeComponent,
                                                                            AudioComponent])


def instantiate_score_and_lives(entities_manager: EntitiesManager, name_to_id_map: Dict[str, int]) -> None:
    text_type_id = entities_manager.add_entity_type([GraphicsComponent, TextComponent])
    name_to_id_map["text_type_id"] = text_type_id

    lives_text_component = TextComponent("Lives: {}".format(INITIAL_PLAYER_LIFE), LIVES_TEXT_SIZE, LIVES_TEXT_COLOR)
    lives_surface = lives_text_component.font.render(lives_text_component.text, False, lives_text_component.color)
    lives_graphics_component = GraphicsComponent(lives_surface, LIVES_POSITION[0], LIVES_POSITION[1])
    ...
    score_text_component = TextComponent("Score: {}".format(INITIAL_PLAYER_SCORE), SCORE_TEXT_SIZE, SCORE_TEXT_COLOR)
    score_surface = score_text_component.font.render(score_text_component.text, False, score_text_component.color)
    score_graphics_component = GraphicsComponent(score_surface, SCORE_POSITION[0], SCORE_POSITION[1])
...
def get_alien_factory(entities_manager: EntitiesManager, alien_type_id: int, alien_surface: pygame.Surface,
                      cyc_surfaces: Tuple[pygame.Surface, ...]) -> Callable[[int, int], int]:
    def alien_factory(initial_x: int, initial_y: int) -> int:
        graphics_compo = GraphicsComponent(alien_surface, initial_x, initial_y)
        ani_cycle_compo = AnimationCycleComponent(cyc_surfaces, ALIEN_ANIMATION_INTERVAL_LENGTH)
        velocity_compo = VelocityComponent(ALIEN_VELOCITY[0], ALIEN_VELOCITY[1])
...
def get_bomb_factory(entities_manager: EntitiesManager, bomb_type_id: int, bomb_surface: pygame.Surface) \
        -> Callable[[int, int], int]:
    def bomb_factory(initial_x: int, initial_y: int) -> int:
        graphics_compo = GraphicsComponent(bomb_surface, initial_x, initial_y)
        velocity_compo = VelocityComponent(BOMB_VELOCITY[0], BOMB_VELOCITY[1])
        return entities_manager.instantiate_entity(bomb_type_id, graphics_compo, velocity_compo)
    return bomb_factory


def get_shot_factory(entities_manager: EntitiesManager, shot_type_id: int, shot_surface: pygame.Surface,
                     shot_sound: pygame.mixer.Sound) -> Callable[[int, int], int]:
    def shot_factory(initial_x: int, initial_y: int) -> int:
        graphics_compo = GraphicsComponent(shot_surface, initial_x, initial_y)
        velocity_compo = VelocityComponent(SHOT_VELOCITY[0], SHOT_VELOCITY[1])
        audio_compo = AudioComponent(shot_sound)
        audio_compo.sound.play()
        return entities_manager.instantiate_entity(shot_type_id, graphics_compo, velocity_compo, audio_compo)
    return shot_factory


def get_explosion_factory(entities_manager: EntitiesManager, explosion_type_id: int, explosion_surface: pygame.Surface,
                          explosion_sound: pygame.mixer.Sound) -> Callable[[int, int], int]:
    def explosion_factory(initial_x: int, initial_y: int) -> int:
        graphics_compo = GraphicsComponent(explosion_surface, initial_x, initial_y)
        lifetime_compo = LifeTimeComponent(EXPLOSION_LIFE_TIME)
        audio_compo = AudioComponent(explosion_sound)
        audio_compo.sound.play()
...
def game_loop(screen: pygame.Surface, background: pygame.Surface, images: List[pygame.Surface],
              sounds: List[pygame.mixer.Sound], entities_manager: EntitiesManager,
              name_to_id_map: Dict[str, int]) -> None:
    ...
    afv_rect = get_component_of_entity(afv, GraphicsComponent).rect
    ...
    while curr_life[0] > 0:
        ...
        dirty_rects.extend(erase_system(screen, background,
                                        entities_manager.get_all_instances_of_component_class(GraphicsComponent)))
        ...
        if aliens_list and random() < BOMB_INSTANTIATION_PROBABILITY:
            last_alien_rect = get_component_of_entity(aliens_list[-1], GraphicsComponent).rect
